...
export const getAllCalendarData = days => {
  const user = Meteor.users.findOne({
    "services.google.email": "mattandkevin1060@gmail.com"
  });
  ...
  GoogleApi.get(
    ...
    (err, res) => {
      ...
      const items = res.items.map(item => ({
        ...
        category: mapSummaryToCategory[item.summary]
      }));
      let calendars = {};
            ...
            new Promise((resolve, reject) => {
              GoogleApi.get(
                ...
                (err, res) => {
                  ...
                  mapIntentToWords[key] = mapIntentToWords[key].concat(
                    res.items.map(i => i.summary).filter(e => e != undefined)
                  );
                  resolve(null);
                }
              );
